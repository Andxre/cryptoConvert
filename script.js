...
function getCurrentPrice(result, quantity) {
  ...
  document.getElementById("currency-input").value = numberWithCommas(
    finalPrice
  );
}
...
function increaseSize(firstLength, secondLength, first, second) {
  ...
  document.getElementById(first).style.width =
    baseWidth + firstLength * 2 + "rem";
  document.getElementById(second).style.width =
    baseWidth + secondLength * 2 + "rem";
}
...
function coinChange() {
  ...
  if (quantity == 0) {
    ...
    // If Input is not a number don't do anything
    return;
  }
...
function changeText(currentCoin) {
  document.getElementById("convert-text").innerHTML =
    currentCoin.charAt(0).toUpperCase() + currentCoin.slice(1) + " to USD";
}
